...
var throttle = require('lodash.throttle'); // Selecting feedback form and saving it in variable


const form = document.querySelector('.feedback-form'); // Setting global variable for input data
...
form.addEventListener('input', throttle(savingInputValues => {
  inputData = {
    email: form.email.value,
    message: form.message.value
  };
  localStorage.setItem('feedback-form-state', JSON.stringify(inputData));
}, 500)); // Initializing listener for feedback form submit event,
